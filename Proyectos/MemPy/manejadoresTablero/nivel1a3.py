...
combinaciones = [] #Lista con las palabras/imagenes ya encontradas.
...
def comparar(window,event,lista,datosJ,tiempo):
    ...
    if len(lista_comparo) == 2:
        ...
        datosJ["Palabra"] = window[event].get_text()
        datosJ["Evento"] = "intento"
        window.refresh()
        if evaluar():
            puntuacion += 3
            combinaciones.append(window[event].get_text())
            datosJ["Estado"] = "ok"
            ...
            window[lista_comparo[0][0]].update("")
            window[lista_comparo[1][0]].update("")  
            lista_comparo.clear()
            puntuacion -= 1
...
def play(user,list_criterios):
    ...
    while True:        
        ...
        if event.startswith("B"):
            ...
            if ok:    
                window[event].update(list_criterios[int(event[1:])-1])
                window[event].update(disabled=True)
                lista_comparo.append((event,window[event].get_text()))
                print(window[event].get_text())
